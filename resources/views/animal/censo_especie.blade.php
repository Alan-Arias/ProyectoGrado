...
<head>
    ...
    <title>Módulo agregar especies</title>
</head>
<body>
@extends('layout.layoutv2')

@section('title', 'Módulo agregar especies')
                ...
                <table class="table table-striped table-bordered table-light">
                    ...
                    @foreach ($Especies as $especie)
                        <tr>
                            ...
                            <td>
                                <a href="{{ url('/CensoRazas/AgregarRaza/' . $especie->id) }}" class="btn btn-info btn-sm">Agregar Raza</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
                    ...
                    @if ($Especies->lastPage() > 1)
                        <ul class="pagination">
                            ...
                            <li class="page-item {{ ($Especies->currentPage() == 1) ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $Especies->previousPageUrl() }}&perPage={{ request('perPage', 1) }}" aria-label="Anterior">&laquo;</a>
                            </li>

                            {{-- Primera página --}}
                            <li class="page-item {{ ($Especies->currentPage() == 1) ? 'active' : '' }}">
                                <a class="page-link" href="{{ $Especies->url(1) }}&perPage={{ request('perPage', 1) }}">1</a>
                            </li>
                            ...
                            @for ($i = max(2, $Especies->currentPage() - 1); $i <= min($Especies->lastPage() - 1, $Especies->currentPage() + 1); $i++)
                                <li class="page-item {{ ($Especies->currentPage() == $i) ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $Especies->url($i) }}&perPage={{ request('perPage', 1) }}">{{ $i }}</a>
                                </li>
                            @endfor
                            ...
                            @if ($Especies->lastPage() > 1)
                                <li class="page-item {{ ($Especies->currentPage() == $Especies->lastPage()) ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $Especies->url($Especies->lastPage()) }}&perPage={{ request('perPage', 1) }}">{{ $Especies->lastPage() }}</a>
                                </li>
                            @endif

                            {{-- Botón "Siguiente" --}}
                            <li class="page-item {{ ($Especies->currentPage() == $Especies->lastPage()) ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $Especies->nextPageUrl() }}&perPage={{ request('perPage', 1) }}" aria-label="Siguiente">&raquo;</a>
                            </li>
                        </ul>
                    @endif
                </div>
